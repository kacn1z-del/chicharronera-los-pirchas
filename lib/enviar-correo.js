...
//
// El correo lleva la factura como IMAGEN adjunta (el diseño con marca
...
import { generarFacturaImagen } from './generar-factura-imagen.js'
 ...
 */
export async function enviarCorreoFactura({ order, clave, numeroConsecutivo, xmlFirmadoBase64, esFactura }) {
  ...
  const qrPngBuffer = await QRCode.toBuffer(clave, {
    width: 480,
    margin: 1,
    errorCorrectionLevel: 'H', // alto — necesario porque el logo tapa el centro del QR
  })
  ...
  await getTransportadorCorreo().sendMail({
    ...
    html:
      ...
      `<img src="cid:factura-imagen" alt="${tipoTexto}" style="max-width:100%;height:auto;display:block;margin-top:12px;" />`,
    attachments: [
      {
